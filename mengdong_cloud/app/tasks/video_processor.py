...
#
# 当 DeepStream（pyds + Gst）不可用时，自动回退到 OpenCV + ultralytics 方式。
...
try:
    ...
    logger.info(
        "DeepStream Python 绑定 (pyds) 不可用，将使用 OpenCV + ultralytics 回退方式"
    )
...
# ============================================================
# OpenCV + ultralytics 回退方式
# ============================================================

class OpenCVVideoTask:
    """使用 OpenCV + ultralytics 处理视频流（DeepStream 不可用时的回退方案）"""
    ...
    def start(self):
        ...
        self._thread.start()
        logger.info("视频任务已启动（OpenCV 模式）: %s", self.analyse_id)
...
# ============================================================
# 统一 VideoTask 工厂 — 自动选择 DeepStream 或 OpenCV 后端
# ============================================================
...
def _create_video_task(
    ...
    """根据运行环境自动选择视频处理后端"""
    if _DEEPSTREAM_AVAILABLE and model_manager.get_ds_config(alg_code):
